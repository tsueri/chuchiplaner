...
| **RecipeTag** | A tag attached to a Recipe. Tags have a `group`: `"season"` (fixed: Frühling/Sommer/Herbst/Winter/Ganzjährig), `"ingredient"` (free-form per household), `"category"` (global controlled vocabulary: Vorspeise/Hauptgericht/Dessert/Snack/Beilage), `"cuisine"` (global controlled vocabulary: Italienisch/Asiatisch/Schweizerisch/…), or `"diet"` (global, multi-value per recipe, vocabulary mirrors schema.org `RestrictedDiet`: `VegetarianDiet`, `VeganDiet`, `GlutenFreeDiet`, `LowFatDiet`, `LowLactoseDiet`, `DiabeticDiet`, `HalalDiet`, `KosherDiet`). One tag of `category` and one of `cuisine` per recipe, enforced by the update endpoint replacing existing tags of those groups on each save. `season` and `diet` are multi-value. The MealSlot's `dietary_filter_tag_id` resolves to a `diet` tag; the week-plan filter therefore matches the schema.org vocabulary. |
